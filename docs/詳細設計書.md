...
作成日：2026年7月28日
対象：`index.html`（約6,700行。うち本体約4,400行＋埋め込んだQRライブラリ約2,300行・スキーマ v5）
...
| `[11]` | `Participant` | Util, Repo, Domain, AppSvc, FB, Ops, App |
| `[12]` | 汎用ヘルパ（`groupBy`・`coverHtml`） | Util |
| `[12b]` | `Wizard` | Util, Repo, Domain, FB, App |
| `[13]` | `App` | Util, Repo, FB, 画面モジュール群 |
| `[14]` | 起動（`boot`） | DB, Repo, App, FB |
...
URL を戻すのは、再読み込みで同じ処理が二重に走らないようにするためです。
...
| R-06 | 本体コードが約4,400行（QRライブラリの埋め込み分約2,300行は別枠） | 区画あたり300行の目安を超えたら分割する。`EventDetail`（約490行）が最大 | 中 |
| R-07 | 一覧が全件読み | `Repo.messages.byEvent` / `tasks.byEvent` を使い、表示対象のイベントぶんだけ読む形に変更する | 小 |
| R-09 | 限定公開でもSNS共有が出る | `renderPromo` で `visibility === "limited"` のときボタンを無効化するか警告を出す | 小 |
| R-10 | PC幅のみ | `min-width:1024px` と `viewport` の指定を外し、主要画面をレスポンシブ化する。影響範囲が広い | 大 |
